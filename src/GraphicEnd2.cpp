...
RESULT_OF_MULTIPNP GraphicEnd2::multiPnP(  vector<PLANE>& plane1, vector<PLANE>& plane2,int id1,int id2, bool loopclosure, int frame_index, int minimum_inliers )
{
...
//        imshow("match", image_matches);
		stringstream ss;
		ss<<"/home/tyu/allwork/slam3d_gx/image/"<<id1<<"-->"<<id2<<".png";
		imwrite(ss.str(), image_matches);
		ss.str("");
		ss.clear();
//		cout<<"KeyFrame: "<<
//        waitKey(_step_time);
